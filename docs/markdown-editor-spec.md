...
| # | 에디터 기능 | 아이콘 라벨 | 분류 | 문법 | HTML 태그 | 동작 타입 |
|---|------------|-----------|------|------|-----------|----------|
| 1 | ATX 제목 | 제목 H1–H6 | Block | `# 텍스트` | `<h1>`–`<h6>` | 줄 접두 |
| 2 | Setext 제목 | 제목(밑줄) | Block | `텍스트` + `===`/`---` | `<h1>`,`<h2>` | 줄 추가 |
| 3 | 인용구 | 인용 | Block | `> 텍스트` | `<blockquote>` | 줄 접두 |
| 4 | 순서 없는 목록 | 글머리 목록 | Block | `- 텍스트` | `<ul><li>` | 줄 접두 |
| 5 | 순서 있는 목록 | 번호 목록 | Block | `1. 텍스트` | `<ol><li>` | 줄 접두 |
| 6 | 펜스 코드 블록 | 코드 블록 | Block | ` ```lang ` | `<pre><code class>` | 블록 삽입 |
| 7 | 들여쓰기 코드 블록 | (자동) | Block | 4칸 들여쓰기 | `<pre><code>` | 줄 접두 |
| 8 | 구분선 | 구분선 | Block | `---` / `***` | `<hr />` | 블록 삽입 |
| 9 | 단락 | (기본) | Block | 일반 텍스트 | `<p>` | 기본 |
| 10 | 굵게 | 굵게(B) | Inline | `**텍스트**` | `<strong>` | 선택 감싸기 |
| 11 | 기울임 | 기울임(I) | Inline | `*텍스트*` | `<em>` | 선택 감싸기 |
| 12 | 굵은 기울임 | 굵게+기울임 | Inline | `***텍스트***` | `<em><strong>` | 선택 감싸기 |
| 13 | 인라인 코드 | 코드 | Inline | `` `텍스트` `` | `<code>` | 선택 감싸기 |
| 14 | 인라인 링크 | 링크 | Inline | `[텍스트](url)` | `<a href>` | 입력 다이얼로그 |
| 15 | 참조 링크 | 참조 링크 | Inline | `[텍스트][id]` | `<a href>` | 입력 다이얼로그 |
| 16 | 이미지 | 이미지 | Inline | `![대체텍스트](url)` | `<img>` | 입력 다이얼로그 |
| 17 | 자동 링크 | 자동 링크 | Inline | `<url>` | `<a href>` | 선택 감싸기 |
| 18 | 강제 줄바꿈 | 줄바꿈 | Inline | 줄 끝 공백 2칸 또는 `\` | `<br />` | 줄 끝 삽입 |
| 19 | 백슬래시 이스케이프 | (수동) | 특수 | `\*` | 리터럴 문자 | 문자 삽입 |
...
| G4 | 자동 링크 리터럴 | `www.example.com` | `<a href>` | remark-gfm 필요 |
