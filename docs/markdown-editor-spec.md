...
> **툴바 반영 (2026-06-27 변경)**: 실제 에디터 툴바는 본 표의 **부분집합**이다. 렌더(파서)는 전 기능을
> 계속 지원하되, 툴바 버튼은 다음 규칙으로 정리한다.
> - **제목 H1–H3만** 제공(H4–H6 제외 — 글자 크기 구분이 약함).
> - **링크 단일화** — 인라인 링크 버튼 1개만. **참조 링크·자동 링크는 툴바 버튼 제외**(문법 입력 시 렌더는 지원).
> - **작업 목록 제외** — 글머리 목록(`-`)과 마커가 겹쳐 툴바에서 뺀다.
> - **추가 제외(2026-06-27)** — Setext 제목(밑줄)·굵은 기울임(`***`)·강제 줄바꿈도 툴바 버튼 제외(렌더는 지원).
> - 툴바는 **그룹(제목·강조·링크·목록·블록) 단위로 묶고 그룹 사이에 구분선**을 둔다. 각 버튼은 테두리 + 클릭 효과.
> - **결과 툴바 15종**: 제목 H1–H3, 굵게·기울임·취소선·인라인 코드, 링크·이미지, 인용·글머리·번호 목록, 코드 블록·표·구분선.

### CommonMark 0.31.2 코어 (19개 기능)

| # | 에디터 기능 | 아이콘 라벨 | 분류 | 문법 | HTML 태그 | 동작 타입 |
|---|------------|-----------|------|------|-----------|----------|
| 1 | ATX 제목 | 제목 H1–H3 *(툴바; 4–6 제외)* | Block | `# 텍스트` | `<h1>`–`<h3>` | 줄 접두 |
| 2 | Setext 제목 | ~~제목(밑줄)~~ *(툴바 제외, 렌더만)* | Block | `텍스트` + `===`/`---` | `<h1>`,`<h2>` | 줄 추가 |
...
| 12 | 굵은 기울임 | ~~굵게+기울임~~ *(툴바 제외, 렌더만)* | Inline | `***텍스트***` | `<em><strong>` | 선택 감싸기 |
| 13 | 인라인 코드 | 코드 | Inline | `` `텍스트` `` | `<code>` | 선택 감싸기 |
| 14 | 인라인 링크 | 링크 | Inline | `[텍스트](url)` | `<a href>` | 입력 다이얼로그 |
| 15 | 참조 링크 | ~~참조 링크~~ *(툴바 제외, 렌더만)* | Inline | `[텍스트][id]` | `<a href>` | 입력 다이얼로그 |
| 16 | 이미지 | 이미지 | Inline | `![대체텍스트](url)` | `<img>` | 입력 다이얼로그 |
| 17 | 자동 링크 | ~~자동 링크~~ *(툴바 제외, 렌더만)* | Inline | `<url>` | `<a href>` | 선택 감싸기 |
| 18 | 강제 줄바꿈 | ~~줄바꿈~~ *(툴바 제외, 렌더만)* | Inline | 줄 끝 공백 2칸 또는 `\` | `<br />` | 줄 끝 삽입 |
...
| G3 | 작업 목록 *(툴바 제외 — `-` 중복, 렌더만)* | `- [ ]` / `- [x]` | `<input type=checkbox>` | remark-gfm 필요 |
