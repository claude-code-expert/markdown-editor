...
|---|------------|-----------|------|------|-----------|----------|
| 1 | ATX 제목 | 제목 H1–H3 ※ | Block | `# 텍스트` | `<h1>`–`<h3>` | 줄 접두 |
...
| 12 | 굵은 기울임 | 굵게+기울임 ✘툴바 | Inline | `***텍스트***` | `<em><strong>` | 선택 감싸기 |
| 13 | 인라인 코드 | 코드 | Inline | `` `텍스트` `` | `<code>` | 선택 감싸기 |
| 14 | 인라인 링크 | 링크 | Inline | `[텍스트](url)` | `<a href>` | 입력 다이얼로그 |
| 15 | 참조 링크 | 참조 링크 ✘툴바 | Inline | `[텍스트][id]` | `<a href>` | 입력 다이얼로그 |
| 16 | 이미지 | 이미지 | Inline | `![대체텍스트](url)` | `<img>` | 입력 다이얼로그 |
| 17 | 자동 링크 | 자동 링크 ✘툴바 | Inline | `<url>` | `<a href>` | 선택 감싸기 |
| 18 | 강제 줄바꿈 | 줄바꿈 ✘툴바 | Inline | 줄 끝 공백 2칸 또는 `\` | `<br />` | 줄 끝 삽입 |
...
| G3 | 작업 목록 | `- [ ]` / `- [x]` | `<input type=checkbox>` | remark-gfm 필요 · **✘툴바**(글머리 목록과 `-` 마커 중복) |
...
`※`/`✘툴바` 표기는 **툴바 버튼 구성**에만 적용된다. 렌더 파이프라인은 모든 기능을 그대로 지원한다(타이핑 시 동작).

**툴바 제외**(렌더는 동작, 버튼만 없음):
- 제목 H4–H6 (글자 크기 구분 약함) → H1–H3만
- 참조 링크(#15) → 링크는 인라인(#14) 1개만
- 작업 목록(G3) → 글머리 목록과 `-` 마커 중복
- 굵은 기울임(#12) → 굵게+기울임 조합으로 대체 가능
- 자동 링크(#17) → 링크로 대체
- 강제 줄바꿈(#18) → 직접 입력

**툴바 = §0 번호순, 16종 / 3그룹**(`group` 경계마다 구분선 `|`):

| 그룹 | 버튼(순서) | group |
|------|-----------|-------|
| ① 코어 블록 (§0 #1–8) | 제목 H1·H2·H3 · Setext · 인용 · 글머리목록 · 번호목록 · 코드블록 · 구분선 | block |
| ② 코어 인라인 (§0 #10–16) | 굵게 · 기울임 · 인라인코드 · 링크 · 이미지 | inline |
| ③ GFM | 취소선 · 표 | special |

- **버튼 스타일** — 각 버튼 1px 테두리 + hover/클릭(press)/활성(active) 상태 효과(anti-slop 준수, 그라데이션·컬러그림자 없음). `globals.css .md-tool-btn`.
